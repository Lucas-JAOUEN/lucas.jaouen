export default {
    data() {
        return {
            en: {
                home: "Home",
                about: "About",
                iam: "I'm",
                mon: "My",
                works: "Works",
                name: "I'm Lucas JAOUEN.",
                metier: "Looking for an internship",
                bio: "I'm working on a Master's Degree in Engineering in my school : the Cnam ENJMIN at Angoulême, France which is the National School of Video Games and Multimedia. I am currently looking for an internship abroad of at least 8 weeks for the summer of 2023.", //I'm a French based web designer & front‑end developer focused on crafting clean & user‑friendly experiences, I am passionate about building excellent software that improves the lives of those around me.
                // bio2: "I am currently looking for an internship abroad of at least 8 weeks for the summer of 2023.",
                labelBtn: "more about me",
                moi: "Me",
                apropos: "About",
                educationContentyear1: "Sept 2021 - Now",
                educationdegree1: "IT & Multimedia Engineer Degree",
                educationinstitute1: "CNAM ENJMIN",
                educationdetails1: `Actually, I am in the second year of my engineering degree in Computer Science and Multimedia`,

                   
                educationContentyear2: "Sept 2019 - Sept 2021",
                educationContentegree2: "DUT INFO (Universitary Diploma of Technology in Computer Science)",
                educationContentinstitute2: "CNAM Paris",
                educationContentdetails2: `Thesis on the Back-End & Front-End development of an
                                internal application and the next year, an other thesis Defence of end of studies`,

             
                educationContentyear3: "Sept 2018 - Nov 2018",
                educationContentdegree3: "DUT GMP (Universitary Diploma of Technology in Mécanic et Production option) ",
                educationContentinstitute3: "CNAM PARIS",
                educationContentdetails3: `Reorientation within the scope of my professional project`,

              
                educationContentyear4: "2015 - 2018",
                educationContentdegree4: "BAC S ",
                educationContentinstitute4: "Lycée la Tour des Dames",
                educationContentdetails4: `Obtained in July 2018`,

                experienceContentyear1: "   Sept 2021 - Present",
                experienceContentposition1: "IT Engineer Apprentice",
                experienceContentcompnayName1: "MIDIPILE",
                experienceContentdetails1: ` - Development & design of an HMI for a new vehicle`,
                experienceContentdetails1bis: `- Implementation of connected services / fleet management`,
    
                
                experienceContentyear2: "Sept 2019 - Sept 2021",
                experienceContentposition2: "Assistant Application Project Manager",
                experienceContentcompnayName2: "AFPA",
                experienceContentdetails2: `- Junior Front-End/Back-End Support/Development Engineer on an internal PANDA application using the Agile Scrum method`,
                experienceContentdetails2bis: `- Integration and acceptance processes for an internal application Magellan, GESPLAN, GESPERS, GESPROD,
                            GESCAN/GESCOVE, before the Production process, creation of tickets if errors`,
                experienceContentdetails2tiers: `- Meetings several times a month to take stock of any developments` ,
        
            
                experienceContentyear3: "2019",
                experienceContentposition3: "Lifeguard",
                experienceContentcompnayName3: "Hotel New Port Bay Club Disney",
                experienceContentdetails3: `- Basin monitoring`,
                experienceContentdetails3bis:`- Welcoming customers in French, English and Spanish`,

                firstName : "Firstname",
                prenomInfo : "Lucas",
                nomInfo : "JAOUEN",
                infoAge : "22 years",
                infoNationalite: "French",
                lastName : "Lastname",
                Age: "Age",
                Nationality: "Nationality",
                internship: "Internship",
                infoInternship: "30/05/22 to 21/07/22",
                address : "Address",
                addressInfo : "Angouleme",
                phone : "Phone",
                phoneInfo : "+33670868818",
                insta : "Insta",
                instaInfo : "lucas_jaouen",
                email : "Email",
                emailInfo : "lucas.jaouen1@gmail.com",
                langages : "Languages",
                langagesInfo : "French, English, Spanish",
                infoPersTitle: "Personal infos",
                skills: "My Skills",

                achievementTitle1 : "3",
                achievementsubTitle1 : "years of",
                achievementsubTitle1bis : "experience",
                
                achievementTitle2 : "10",
                achievementsubTitle2 : "completed",
                achievementsubTitle2bis : "projects",

                achievementTitle3 : "30",
                achievementsubTitle3 : "happy",
                achievementsubTitle3bis : "associates",

                achievementTitle4 : "200",
                achievementsubTitle4 : "nice",
                achievementsubTitle4bis : "meeting",
                enter : "get in",
                touch : "touch",
                contactTitle : "Don't be shy !",
                paragContact : "Feel free to get in touch with me. I am always open to discussing new projects, creative ideas or opportunities to be part of your visions.",
                addressCarte : "ADDRESS POINT",
                mailMe : "mail me",
                callMe : "call me",
                labelBtnSendMail : "Send Message",
                placeholderName : "Your name",
                placeholderEmail : "Your email",
                placeholderSubject : "Your subject",
                placeholderMessage : "Your message",
                labelDownloadCV : "Download CV",
                alertmsg: "Message envoyé !",

                blogs: [
                    {
                      id: 1,
                      img: require(`@/assets/images/blog/blog-post-1.jpg`),
                      title: "A New Map Shows how Solar Winds Rain Down Everywhere on Mars",
                      commentor: "SCOTT ALAN JOHNSTON",
                      date: "05 September 2022",
                      tag: `universe, mars, solar, winds`,
                      description1:
                        "In a joint effort between NASA’s MAVEN spacecraft and the United Arab Emirates’ Emirates Mars mission (EMM), scientists have observed an uncommonly chaotic interaction between the solar wind and Mars’ upper atmosphere, creating a unique ultraviolet aurora. The phenomenon represents an unusual occurrence in Martian space weather, and scientists are excited to take advantage of future collaborations between spacecraft to keep an eye out for repeat events.",
                      description2:
                        "EMM’s observations suggested that the aurora was so widespread and disorganized that the plasma environment around Mars must have been truly disturbed, to the point that the solar wind was directly impacting the upper atmosphere wherever we observed auroral emission…By combining EMM auroral observations with MAVEN measurements of the auroral plasma environment, we can confirm this hypothesis and determine that what we were seeing was essentially a map of where the solar wind was raining down onto the planet.",
                      description3:
                        "Essentially, it was a temporary breakdown of Mars’ natural defences against solar radiation, when particles were able to take advantage of the chaotic space weather to find a way down to the planet’s atmosphere.",
                      description4:
                        "MAVEN arrived at Mars in 2014 and was joined by EMM in 2021. There are more than half a dozen probes in Martian orbit, each with different specialties and capabilities. Working together, they can help us understand Mars in a way that they cannot do on their own, including studying its unique auroras.",
                    },
                    {
                      id: 2,
                      img: require(`@/assets/images/blog/blog-post-2.jpg`),
                      title: "Top 10 Toolkits for Deep Learning in 2022",
                      commentor: "Santhan ",
                      date: "14 January 2022",
                      tag: `wordpress, business, economy, design`,
                      description1:
                        "Consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
                      description2:
                        "If you’ve been waiting for an invitation, this calligraphy is it. Commissioned by Notebook hand-lettered design for a poster. Quote is Notebook Building 8 VP’s Regina Dugan—and mine. ",
                      description3:
                        "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.",
                      description4:
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
                    },
                    {
                      id: 3,
                      img: require(`@/assets/images/blog/blog-post-3.jpg`),
                      title: "Everything You Need to Know About Web Accessibility",
                      commentor: "steve ",
                      date: "9 January 2020",
                      tag: `wordpress, business, economy, design`,
                      description1:
                        "Dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
                      description2:
                        "Most photographers find it hard to see interesting pictures in places in which they are most familiar. A trip somewhere new seems always exactly what our photography needed, as shooting away from home consistently inspires us to new artistic heights. ",
                      description3:
                        "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.",
                      description4:
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
                    },
            
                    {
                      id: 4,
                      img: require(`@/assets/images/blog/blog-post-4.jpg`),
                      title: "How to Inject Humor & Comedy Into Your Brand",
                      commentor: "Beker ",
                      date: "15 March 2022",
                      tag: `wordpress, business, economy, design`,
                      description1:
                        "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas",
                      description2:
                        "Most photographers find it hard to see interesting pictures in places in which they are most familiar. A trip somewhere new seems always exactly what our photography needed, as shooting away from home consistently inspires us to new artistic heights. ",
                      description3:
                        "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.",
                      description4:
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
                    },
                    {
                      id: 5,
                      img: require(`@/assets/images/blog/blog-post-5.jpg`),
                      title: "Women in Web Design: How To Achieve Success",
                      commentor: "Janntul ",
                      date: "9 January 2021",
                      tag: `wordpress, business, economy, design`,
                      description1:
                        "Jorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
                      description2:
                        "Most photographers find it hard to see interesting pictures in places in which they are most familiar. A trip somewhere new seems always exactly what our photography needed, as shooting away from home consistently inspires us to new artistic heights. ",
                      description3:
                        "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.",
                      description4:
                        "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
                    },
                    {
                      id: 6,
                      img: require(`@/assets/images/blog/blog-post-6.jpg`),
                      title: "Evergreen versus topical content: An overview",
                      commentor: "Hasan ",
                      date: "9 January 2022",
                      tag: `wordpress, business, economy, design`,
                      description1:
                        "Still ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.",
                      description2:
                        "Most photographers find it hard to see interesting pictures in places in which they are most familiar. A trip somewhere new seems always exactly what our photography needed, as shooting away from home consistently inspires us to new artistic heights. ",
                      description3:
                        "Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.",
                      description4:
                        "Still ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
                    },
                  ],

            }
        }
    }
}